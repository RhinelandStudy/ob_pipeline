...
class MultiProcPlugin(DistributedPluginBase):
    ...
    def __init__(self, plugin_args=None):
        ...
        logger.debug('MultiProcPlugin starting in "%sdaemon" mode (n_procs=%d, mem_gb=%0.2f',
                                                                   ' ngpus=%d)',
                     'not non-daemon' , self.processors, self.memory_gb, self.n_gpus)
    ...
    def _submit_job(self, node, devno=None, updatehash=False):
        ...
        #return self._taskid
        #37
        result_future = self.pool.submit(run_node, node, updatehash, self._taskid,devno)
    ...
    def _send_procs_to_workers(self, updatehash=False, graph=None):
        ...
        if self._stats != stats:
            ...
            logger.info(
                ...
                "memory (GB): %0.2f/%0.2f, Free processors: %d/%d Free GPU Slots:%d/%d.%s",
        ...
        for jobid in jobids:
            ...
            logger.debug( "Allocating %s ID=%d (%0.2fGB, %d threads). Free: " "%0.2fGB, %d threads.", self.procs[jobid].fullname,
                jobid, next_job_gb, next_job_th, free_memory_gb, free_processors,)
            ...
            if is_gpu_job:
                ...
                if next_job_gpu_th > 1:
                    ...
                    if is_gpu_free and next_job_gpu_th <= len(slotnos):
                        ...
                        #py37 is unhappy here with json dumps int64, we have to convert to int 
                        #logger.debug('[*** GPU ID: %d  Running Job: %s Job-ID: %d on multiple slots:%s, Queue State:%s' % (devno, self.procs[jobid]._id, jobid, slotnos, json.dumps(self.gpu_q)))
                else:
                    ...
                    if is_gpu_free and slotno is not None:
                        ...
                        #py37 is unhappy here with json dumps int64, we have to convert to int
                        #logger.debug('[*** GPU ID: %d  Running Job: %s Job-ID: %d on single slot-ID:%d, Queue State:%s' % (devno, self.procs[jobid]._id, jobid, slotno, json.dumps(self.gpu_q)))
